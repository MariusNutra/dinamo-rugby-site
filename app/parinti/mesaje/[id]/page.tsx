...
export default function ParintiConversationPage() {
  ...
  const adminParticipants = conversation?.participants.filter(p => p.type === 'admin').map(p => p.name).join(', ')

  return (
    <div className="max-w-2xl mx-auto flex flex-col h-[calc(100vh-80px)]">
      {/* Header */}
      <div className="flex items-center gap-3 mb-4">
        <Link href="/parinti/mesaje" className="text-gray-400 hover:text-gray-600">
        ...
        <div>
          <h1 className="font-heading font-bold text-lg">
            {conversation?.subject || adminParticipants || 'Conversație'}
          </h1>
          {conversation?.subject && adminParticipants && (
            <p className="text-xs text-gray-500">{adminParticipants}</p>
          )}
        </div>
      </div>
        ...
        {messages.map(msg => (
          <div key={msg.id} className={`flex ${msg.isMe ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 ${
              msg.isMe
                ? 'bg-dinamo-red text-white rounded-br-md'
                : 'bg-white text-gray-900 shadow-sm rounded-bl-md'
            }`}>
              {!msg.isMe && (
                <p className="text-xs font-medium text-dinamo-blue mb-1">{msg.senderName}</p>
              )}
